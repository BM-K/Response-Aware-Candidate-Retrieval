...
from transformers.modeling_outputs import (BaseModelOutput,
                                           ...
                                           Seq2SeqLMOutput,
                                           Seq2SeqModelOutput)
from transformers.models.blenderbot_small import (BlenderbotSmallForConditionalGeneration,
...
class BlenderbotEncoder(BlenderbotPreTrainedModel):
    ...
    def forward(
        ...
        if len(input_ids.size()) == 3: use_response = True
        else: use_response = False

        if use_response:
            b = input_ids.size(0)
            n_passages = input_ids.size(1)
            input_ids = input_ids.view(int(b*n_passages), -1)
        ...
        if attention_mask is not None:
            if use_response:
                attention_mask = attention_mask.view(int(b*n_passages), -1)
        ...
        if use_response:
            hidden_states = hidden_states.view(b, -1, self.embed_dim)
...
class BlenderbotDecoder(BlenderbotPreTrainedModel):
    ...
    def forward(
        ...
        if len(encoder_attention_mask.size()) == 3: use_response = True
        else: use_response = False

        if use_response:
            b = encoder_attention_mask.size(0)
            n_passages = encoder_attention_mask.size(1)
...
class BlenderbotConditionalGeneration(nn.Module):
    def __init__(self, args, tokenizer):
        super(BlenderbotConditionalGeneration, self).__init__()
        self.args = args
        ...
        self.model = BlenderbotForConditionalGeneration.from_pretrained("facebook/blenderbot-1B-distill")
    ...
    def forward(self, inputs, mode):
        ...
        if mode != 'test':
            if self.args.use_db_response == 'True':
                ...
                if mode == 'train' and random.random() < self.args.alpha_blending:
                    outs = self.model(input_ids=inputs['input_ids'],
                                      attention_mask=inputs['attention_mask'],
                                      decoder_input_ids=inputs['bl_decoder_input_ids'],
                                      decoder_attention_mask=inputs['bl_decoder_attention_mask'],
                                      labels=inputs['bl_labels'], return_dict=True,
                                      use_response=True,
                                      args=self.args)
                    loss = self.loss_fct(outs.logits.view(-1, self.vocab_size), inputs['bl_labels'].view(-1))
                else:
                    outs = self.model(input_ids=inputs['input_ids'],
                                      attention_mask=inputs['attention_mask'],
                                      ...
                                      use_response=True,
                                      args=self.args)
                    loss = self.loss_fct(outs.logits.view(-1, self.vocab_size), inputs['labels'].view(-1))
            ...
            if self.args.sampling_method == 'beam':
                outputs = self.model.generate(inputs['input_ids'],
                                              max_length=self.args.max_len,
                                              encoder_no_repeat_ngram_size=0,
                                              num_beams=1)

            elif self.args.alpha_blending == 'sampling':
                outputs = self.model.generate(inputs['input_ids'],
                                              do_sample=True,
                                              max_length=self.args.max_len,
                                              encoder_no_repeat_ngram_size=0,
